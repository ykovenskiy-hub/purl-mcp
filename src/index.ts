...
async function readPurlFile(filePath: string): Promise<PurlProject> {
  const absolutePath = path.resolve(filePath)
  const content = await fs.readFile(absolutePath, 'utf-8')
  return JSON.parse(content) as PurlProject
}

// Helper: Write .purl file atomically (write to tmp then rename)
async function writePurlFile(filePath: string, project: PurlProject): Promise<void> {
  const absolutePath = path.resolve(filePath)
...
function getAllObjectNames(project: PurlProject): Set<string> {
...
function removeObjectFromList(objects: CellObject[], name: string): CellObject[] {
  return objects
...
function findObject(project: PurlProject, name: string, cellName?: string): { object: CellObject; cell: PurlCell } | null {
  ...
  for (const cell of cells) {
    ...
    if (found) return { object: found, cell }
  }
  return null
}

function findObjectInList(objects: CellObject[], name: string): CellObject | null {
...
function getAllObjects(objects: CellObject[], results: CellObject[] = []): CellObject[] {
  for (const obj of objects) {
    results.push(obj)
    if (isComponent(obj)) {
...
function summarizeObject(obj: CellObject): Record<string, unknown> {
  const summary: Record<string, unknown> = {
    name: obj.name,
    type: isComponent(obj) ? 'component' : (obj as any).primeType,
    visible: obj.visible,
  }

  if (obj.tags?.length) summary.tags = obj.tags
  if (obj.template) summary.template = true
  if (obj.movable) summary.movable = true
  if (obj.draggable) summary.draggable = true
  if (obj.blocking) summary.blocking = obj.blocking
  if (obj.snapToGrid) summary.snapToGrid = obj.snapToGrid
  if (isComponent(obj)) summary.childCount = obj.children.length
...
// Tool handlers
async function handleGetProject(args: { path: string }): Promise<string> {
  ...
  const summary = {
    ...
    startCell: project.cells.find(c =>
      project.markers.some(m => m.type === 'start' && m.cellId === c.id)
    )?.label,
...
async function handleListObjects(args: { path: string; cellName?: string }): Promise<string> {
  const project = await readPurlFile(args.path)
...
async function handleGetScript(args: { path: string; target: string }): Promise<string> {
  const project = await readPurlFile(args.path)
...
async function handleValidateScript(args: { code: string }): Promise<string> {
...
async function handleListPurlFiles(args: { directory?: string }): Promise<string> {
...
async function handleAddObject(args: {
  ...
  cellName: string
  name: string
  type: 'shape' | 'text' | 'line' | 'grid' | 'component'
  properties?: Record<string, unknown>
}): Promise<string> {
  const project = await readPurlFile(args.path)
...
async function handleRemoveObject(args: {
  ...
  const childCount = wasComponent ? (found.object as Component).children.length : 0

  found.cell.interior.objects = removeObjectFromList(
    found.cell.interior.objects,
    args.objectName
  )
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case 'get_project':
        result = await handleGetProject(args as { path: string })
        break
      case 'list_objects':
        result = await handleListObjects(args as { path: string; cellName?: string })
        break
      case 'get_script':
        result = await handleGetScript(args as { path: string; target: string })
        break
      case 'validate_script':
        result = await handleValidateScript(args as { code: string })
        break
      case 'list_purl_files':
        result = await handleListPurlFiles(args as { directory?: string })
        break
      case 'update_script':
        result = await handleUpdateScript(args as {
          path: string; target: string; code: string; scriptName?: string
        })
        break
      case 'set_property':
        result = await handleSetProperty(args as {
          path: string; objectName: string; cellName?: string; properties: Record<string, unknown>
        })
        break
      case 'add_object':
        result = await handleAddObject(args as {
          path: string; cellName: string; name: string;
          type: 'shape' | 'text' | 'line' | 'grid' | 'component';
          properties?: Record<string, unknown>
        })
        break
      case 'remove_object':
        result = await handleRemoveObject(args as {
          path: string; objectName: string; cellName?: string
        })
        break
      case 'update_cell':
        result = await handleUpdateCell(args as {
          path: string; cellName: string; properties: Record<string, unknown>
        })
        break
      default:
